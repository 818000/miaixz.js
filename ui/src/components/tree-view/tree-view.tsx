...
/**
 * Renders the public generic TreeView implementation.
 *
 * @typeParam Value - Consumer-owned value attached to each node.
 ...
 */
function TreeViewImplementation<Value = unknown>(
  props: TreeViewProps<Value>,
    ...
    siblings.map((node) => {
      ...
      return (
        <div
          ...
          className="miaixz-tree-view-item"
        ...
        >
          <div
            className="miaixz-tree-view-row"
            onClick={() => {
              focusNode(node.id);
          ...
          >
            <span
              className="miaixz-tree-view-expander"
            ...
            <span className="miaixz-tree-view-label">{node.label}</span>
            {selected && selectionMode !== "none" && (
              <Icon name="Check" size="control" className="miaixz-tree-view-selection" />
            )}
          </div>
          {failed && (
            <div className="miaixz-tree-view-error" role="alert">
              <span>{t("ui.tree.loadError")}</span>
              <button
                type="button"
                className="miaixz-tree-view-retry"
                onClick={(event) => {
                  event.stopPropagation();
          ...
          {expanded && resolvedChildren !== undefined && resolvedChildren.length > 0 && (
            <div role="group" className="miaixz-tree-view-group">
  ...
  return (
    <div
      ...
      className={classNames("miaixz-tree-view", className)}
 ...
 * @public
 */
export const TreeView = forwardRef(TreeViewImplementation) as <Value = unknown>(
  props: TreeViewProps<Value> & RefAttributes<HTMLDivElement>,
) => ReactElement;
