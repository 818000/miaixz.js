...
/**
 * Describes one business-neutral node rendered by TreeView.
 *
...
/**
 * Defines Miaixz-owned TreeView properties before native div attributes are merged.
 *
 * @typeParam Value - Optional consumer-owned value attached to each node.
 * @public
 */
export interface MiaixzTreeViewOwnProps<Value = unknown> {
  /**
 ...
 * @public
 */
export interface TreeViewProps<Value = unknown>
  extends
    Omit<HTMLAttributes<HTMLDivElement>, keyof MiaixzTreeViewOwnProps<Value>>,
    MiaixzTreeViewOwnProps<Value> {}
