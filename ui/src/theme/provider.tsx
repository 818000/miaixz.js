...
import {
  readMiaixzSystemDarkPreference,
  resolveMiaixzColorMode,
  subscribeMiaixzSystemDarkPreference,
} from "./mode.js";
import { mergeMiaixzThemeComponents } from "./registry.js";
import { hasThemePreset, loadThemePreset } from "./presets/index.js";
import { serializeThemeApplication } from "./serialize.js";
import type { MiaixzThemeLoader, ThemeContextValue, ThemeProps } from "./types.js";
...
export function Theme(props: ThemeProps) {
  const { appearance, loader, fallback = miaixzDefaultAppearance.theme, nonce, children } = props;
  ...
  const canLoadTheme = useCallback(
    (name: string) => hasThemePreset(name) || loader !== undefined,
    [loader],
  );
  const activeLoader = useCallback<MiaixzThemeLoader>(
    (name, context) => {
      if (hasThemePreset(name)) return loadThemePreset(name, context);
      if (loader !== undefined) return loader(name, context);
      return Promise.reject(new MiaixzThemeError("UI_THEME_NOT_FOUND", { theme: name }));
    },
    [loader],
  );
  ...
  const [runtime, setRuntime] = useState<ThemeRuntimeState>(() =>
    catalog.has(appearance.getSnapshot().theme)
      ? { status: "ready" }
      : !canLoadTheme(appearance.getSnapshot().theme)
        ? {
            status: "error",
  ...
  const awaitingInitialPreset =
    hasThemePreset(snapshot.theme) && !catalog.has(snapshot.theme) && runtime.status === "loading";
  const resolvedColorMode = resolveMiaixzColorMode(snapshot.colorMode, systemDark);
  ...
  useMiaixzLayoutEffect(() => {
    if (awaitingInitialPreset) return undefined;
  ...
  }, [application, awaitingInitialPreset, scope]);
  ...
  const commit = useCallback(
    (next: MiaixzAppearanceSettings, persist: () => void): void => {
      const nextMode = resolveMiaixzColorMode(next.colorMode, readMiaixzSystemDarkPreference());
  ...
  const setTheme = useCallback(
    ...
    [activeLoader, appearance, canLoadTheme, catalog, commit],
  );
  ...
  useEffect(() => {
    ...
    if (!canLoadTheme(snapshot.theme)) {
  ...
  }, [appearance, canLoadTheme, catalog, fallback, setTheme, snapshot.theme]);
